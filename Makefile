...
# USE THE FOLLOWING 2 LINES IF COMPILING ON A MAC MACHINE
SRCFLS = $(shell gfind . -regex ".*\.cpp" -printf "%f ")
SRCDIR = $(shell gfind . -regex ".*\.cpp" -printf "%h\n" | sort -u | tr '\n' ':')

# USE THE FOLLOWING 2 LINES IF COMPILING ON A LINUX MACHINE
